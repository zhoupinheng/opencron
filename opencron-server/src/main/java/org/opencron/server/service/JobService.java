...
import org.opencron.common.job.Opencron.CronType;
import org.opencron.common.job.Opencron.ExecType;
import org.opencron.common.job.Opencron.JobType;
import org.opencron.common.job.Opencron.RunStatus;
...
public class JobService {
  ...
  public List<JobVo> getJobVo(ExecType execType, CronType cronType) {
    String sql = "SELECT T.*,D.name AS agentName,D.port,D.ip,D.password FROM T_JOB AS T " + "LEFT JOIN T_AGENT AS D " + "ON T.agentId = D.agentId " + "WHERE IFNULL(T.flowNum,0)=0 " + "AND cronType=? "
        + "AND execType=? " + "AND T.deleted=0 " + "AND IFNULL(T.pause,0)=0";
  ...
  public List<JobVo> getJobVoByAgentId(Agent agent, ExecType execType, CronType cronType) {
    String sql = "SELECT T.*,D.name AS agentName,D.port,D.ip,D.password FROM T_JOB AS T " + "INNER JOIN T_AGENT D " + "ON T.agentId = D.agentId " + "WHERE IFNULL(T.flowNum,0)=0 " + "AND cronType=? "
        + "AND execType = ? " + "AND T.deleted=0 " + "AND IFNULL(T.pause,0)=0 " + "AND D.agentId=? ";
  ...
  public PageBean<JobVo> getJobVos(HttpSession session, PageBean pageBean, JobVo job) {
    String sql = "SELECT T.*,D.name AS agentName,D.port,D.ip,D.password,U.userName AS operateUname " + "FROM T_JOB AS T " + "LEFT JOIN T_AGENT AS D " + "ON T.agentId = D.agentId "
        + "LEFT JOIN T_USER AS U " + "ON T.userId = U.userId " + "WHERE IFNULL(flowNum,0)=0 " + "AND T.deleted=0 ";
    if (job != null) {
  ...
  private List<JobVo> queryChildren(JobVo job) {
    if (job.getJobType().equals(JobType.FLOW.getCode())) {
      String sql = "SELECT T.*,D.name AS agentName,D.port,D.ip,D.password,U.userName AS operateUname FROM T_JOB AS T " + "LEFT JOIN T_AGENT AS D " + "ON T.agentId = D.agentId "
          + "LEFT JOIN T_USER AS U " + "ON T.userId = U.userId " + "WHERE T.deleted=0 " + "AND T.flowId = ? " + "AND T.flowNum>0 " + "ORDER BY T.flowNum ASC";
  ...
  public JobVo getJobVoById(Long id) {
    String sql = "SELECT T.*,D.name AS agentName,D.port,D.ip,D.password,U.username AS operateUname " + " FROM T_JOB AS T " + "LEFT JOIN T_AGENT AS D " + "ON T.agentId = D.agentId "
        + "LEFT JOIN T_USER AS U " + "ON T.userId = U.userId " + "WHERE T.jobId =?";
  ...
  public List<JobVo> getJobByAgentId(Long agentId) {
    String sql = "SELECT T.*,D.name AS agentName,D.port,D.ip,D.password,U.userName AS operateUname FROM T_JOB AS T " + "LEFT JOIN T_USER AS U " + "ON T.userId = U.userId " + "LEFT JOIN T_AGENT D "
        + "ON T.agentId = D.agentId " + "WHERE T.agentId =?";
  ...
  public String checkDelete(Long id) {
    ...
    if (job.getJobType() == JobType.FLOW.getCode()) {
      sql = "SELECT COUNT(1) FROM T_RECORD AS R INNER JOIN (" + " SELECT J.jobId FROM T_JOB AS J INNER JOIN T_JOB AS F" + " ON J.flowId = F.flowId" + " WHERE f.jobId = ?" + " ) AS J"
          + " on R.jobId = J.jobId" + " and R.status=?";
